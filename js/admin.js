(function () {
  ...
  function renderKPIs() {
    const kpis = [
      { icon: '📊', value: 128, label: 'Total Active Issues', trend: '↑ 8% week' },
      { icon: '🔴', value: 14, label: 'Critical Issues', trend: '↑ 3 today' },
      { icon: '🆕', value: 23, label: 'New Reports', trend: 'Last 24h' },
      { icon: '🧩', value: clusters.length, label: 'Duplicate Clusters', trend: 'Review needed' },
      { icon: '👤', value: assignments.filter(a => !a.assigned).length, label: 'Pending Assignments', trend: 'Action needed' },
      { icon: '⏰', value: 7, label: 'Nearing SLA', trend: 'Next 6 hours' },
      { icon: '✅', value: 86, label: 'Resolved Issues', trend: 'This month' },
      { icon: '🔁', value: 5, label: 'Reopened Issues', trend: 'Needs review' }
    ];
    document.getElementById('kpiGrid').innerHTML = kpis.map((k, i) => `
      <div class="stat-card" style="animation-delay:${i * 0.04}s" data-kpi="${k.label}">
        <div class="stat-icon">${k.icon}</div>
        <div class="stat-value">${k.value}</div>
        <div class="stat-label">${k.label}</div>
        <div class="stat-trend">${k.trend}</div>
      </div>
    `).join('');
    document.querySelectorAll('#kpiGrid .stat-card').forEach(card => {
      card.addEventListener('click', () => {
        const label = card.dataset.kpi;
        if (label.includes('Critical') || label.includes('Priority')) document.getElementById('priority').scrollIntoView({ behavior: 'smooth' });
        else if (label.includes('Cluster')) document.getElementById('clusters').scrollIntoView({ behavior: 'smooth' });
        else if (label.includes('Assignment')) document.getElementById('assignments').scrollIntoView({ behavior: 'smooth' });
        else if (label.includes('SLA')) document.getElementById('sla').scrollIntoView({ behavior: 'smooth' });
        else document.getElementById('issues').scrollIntoView({ behavior: 'smooth' });
        CR.toast(`Filtered view: ${label}`, 'success');
      });
    });
  }
    ...
    try {
      ...
      if (dash.metrics) {
        ...
        document.getElementById('kpiGrid').innerHTML = [
          { icon: '📊', value: m.total_active_issues, label: 'Total Active Issues', trend: 'Live' },
          { icon: '🔴', value: m.critical_issues, label: 'Critical Issues', trend: 'Live' },
          { icon: '🆕', value: m.new_reports, label: 'New Reports', trend: 'Live' },
          { icon: '🧩', value: clusters.length, label: 'Duplicate Clusters', trend: 'Review' },
          { icon: '👤', value: m.pending_assignments, label: 'Pending Assignments', trend: 'Action' },
          { icon: '⏰', value: m.sla_due_today, label: 'Nearing SLA', trend: 'Due today' },
          { icon: '✅', value: m.resolved_issues, label: 'Resolved Issues', trend: 'Live' },
          { icon: '🔁', value: m.reopened_issues, label: 'Reopened Issues', trend: 'Live' }
        ].map((k, i) => `
